...
# Create a window
win = pg.GraphicsLayoutWidget(show=True, title="Graphs")
win.resize(800, 600)
...
# Plot the data
plot.plot(x, y, pen="b")  # 'b' sets the color to blue
def update():
    global data
    data = np.roll(data, -1)      # Shift data
    data[-1] = np.random.normal() # Add new random data point
    plot.plot(data, clear=True)   # Clear the old plot and add new data
    state = client.state()
    markets = {}
